...
func (ur *UserRepository) InsertUser(user *domain.User) (*domain.User, error) {
	stmt, err := ur.db.Prepare(
		`INSERT INTO user(name, facebook_id, photo, email) VALUES(?, ?, ?, ?)`)
	...
	result, err := stmt.Exec(user.Name, user.FacebookId, user.Picture, user.Email)

	if err != nil {
		log.Printf("Error [InsertUser]: %v - [%v %v %v %v]", err, user.Name, user.FacebookId, user.Picture, user.Email)
		return nil, err
	}
