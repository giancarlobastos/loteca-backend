...
func (ur *UserRepository) InsertUser(user *domain.User) (*domain.User, error) {
	stmt, err := ur.db.Prepare(
		`INSERT INTO user(name, facebook_id, photo) VALUES(?, ?, ?)`)
	...
	result, err := stmt.Exec(user.Name, user.FacebookId, user.PhotoUrl)

	if err != nil {
		log.Printf("Error [InsertUser]: %v - [%v %v %v]", err, user.Name, user.FacebookId, user.PhotoUrl)
		return nil, err
	}
...
func (ur *UserRepository) GetUser(id int) (*domain.User, error) {
	return ur.getUser(
		`SELECT id, name, facebook_id, photo
		 FROM user
		 WHERE id = ?`, &id)
}
...
func (ur *UserRepository) getUser(query string, args ...interface{}) (*domain.User, error) {
	...
	if rows.Next() {
		user := domain.User{}
		err = rows.Scan(&user.Id, &user.Name, &user.FacebookId, &user.PhotoUrl)
